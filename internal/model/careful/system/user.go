...
type User struct {
	...
	Username    string           `gorm:"size:64;not null;uniqueIndex;column:username;comment:用户名" json:"username"`                  // 用户名
	Password    string           `gorm:"size:512;not null;column:password;comment:密码" json:"-"`                                     // 密码
	Gender      user.GenderConst `gorm:"type:tinyint;default:1;column:gender;comment:性别" json:"gender"`                             // 性别
	Email       string           `gorm:"size:32;index;column:email;comment:邮箱" json:"email"`                                        // 邮箱
	Mobile      string           `gorm:"size:11;index;column:mobile;comment:手机号" json:"mobile"`                                     // 手机号
	Name        string           `gorm:"size:32;index;column:name;comment:真实姓名" json:"name"`                                        // 真实姓名
	...
	City        string           `gorm:"size:64;column:city;comment:所在城市" json:"city"`                                              // 所在城市
	Address     string           `gorm:"size:128;column:address;comment:详细地址" json:"address"`                                       // 详细地址
	Bio         string           `gorm:"size:256;column:bio;comment:个人简介" json:"bio"`                                               // 个人简介
	...
	// manager 直属上级
	ManagerID *string `gorm:"size:40;column:manager_id;comment:直属上级ID" json:"manager_id"` // 上级菜单ID
	Manager   *User   `gorm:"foreignKey:ManagerID" json:"manager"`                        // 直属上级
	// 所属部门
	DeptID *string `gorm:"size:40;index;column:dept_id;comment:所属部门ID" json:"dept_id"`                  // 所属部门ID
