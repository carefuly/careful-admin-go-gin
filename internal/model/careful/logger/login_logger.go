...
type LoginLogger struct {
	...
	LoginUsername  string `gorm:"size:40;index;column:login_username;comment:登录用户名" json:"login_username"`                   // 登录用户名
	...
	FailureReason  string `gorm:"size:255;column:failure_reason;comment:失败结果" json:"failure_reason"`                         // 失败原因
}
