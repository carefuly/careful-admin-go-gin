/**
 * Description：
 * FileName：role_logging_decorator.go
 * Author：CJiaの用心
 * Create：2025/12/19 22:10:10
 * Remark：
 */
...
type RoleCacheLoggingDecorator struct {
	cache  cacheSystem.RoleCache
	logger cacheRecord.CacheLogger
}

func NewRoleCacheLoggingDecorator(cache cacheSystem.RoleCache, logger cacheRecord.CacheLogger) RoleCacheLoggingDecorator {
	return RoleCacheLoggingDecorator{
		cache:  cache,
		logger: logger,
	}
}
...
// 通用日志记录函数
func (d *RoleCacheLoggingDecorator) logOperation(
	ctx context.Context,
	key string,
	value interface{},
	err error,
...
// 从上下文中安全获取字符串值
func (d *RoleCacheLoggingDecorator) getStringFromContext(ctx context.Context, key string) string {
...
func (d *RoleCacheLoggingDecorator) Get(ctx context.Context, id string) (*domainSystem.Role, error) {
	start := time.Now()
	result, err := d.cache.Get(ctx, id)

	// 特殊处理"未找到"情况
	var value interface{}
	if errors.Is(err, cacheSystem.ErrRoleNotExist) {
		value = "not_found"
	} else if result != nil {
		value = result
	}
...
func (d *RoleCacheLoggingDecorator) Set(ctx context.Context, domain domainSystem.Role) error {
	start := time.Now()
...
func (d *RoleCacheLoggingDecorator) Del(ctx context.Context, id string) error {
	start := time.Now()
...
func (d *RoleCacheLoggingDecorator) SetNotFound(ctx context.Context, id string) error {
	start := time.Now()
...
func (d *RoleCacheLoggingDecorator) key(id string) string {
	return fmt.Sprintf("%s:%s", cacheSystem.ErrRoleKey, id)
}
