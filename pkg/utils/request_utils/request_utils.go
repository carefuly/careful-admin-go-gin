...
// GetNormalizedRequestIP 获取真实IP并进行标准化处理
func GetNormalizedRequestIP(c *gin.Context) string {
	var ip string

	// 1. 优先获取 X-Forwarded-For 最后一个IP（兼容代理场景）
	xff := c.Request.Header.Get("X-Forwarded-For")
	if xff != "" {
		ipParts := strings.Split(xff, ",")
		if len(ipParts) > 0 {
			ip = strings.TrimSpace(ipParts[len(ipParts)-1])
		}
	}

	// 2. 若无，使用 Gin 内置的 ClientIP（处理 RemoteAddr、X-Real-IP 等）
	if ip == "" {
		ip = c.ClientIP()
	}

	// 3. 若无，使用自定义的 request_ip
	if ip == "" {
		if requestIP, exists := c.Get("request_ip"); exists {
			if ipStr, ok := requestIP.(string); ok {
				ip = ipStr
			}
		}
	}

	// 4. 标准化处理（合并 NormalizeIP 的逻辑）
	ip = normalizeIP(ip)
...
// 提取 NormalizeIP 的核心逻辑，用于单独格式化IP
